  ...
  "col-start-2 col-end-3 row-start-1 row-end-1 flex size-4 items-center justify-center rounded-full border border-current",
  {
    variants: {
      status: {
        ...
        done: "bg-primary border-primary [&>*:not(.lucide-check)]:hidden [&>.lucide-check]:text-white",
        error:
          "border-destructive bg-destructive [&>*:not(.lucide-x)]:hidden [&>.lucide-x]:text-white",
...
const timelineHeadingVariants = cva("row-start-1 row-end-1 line-clamp-1 max-w-full truncate", {
  variants: {
    ...
    variant: {
      primary: "text-sm font-bold text-foreground",
...
const TimelineLine = React.forwardRef<HTMLHRElement, TimelineLineProps>(
  ({ className, done = false, ...props }, ref) => (
    <hr
      ...
      className={cn(
        "col-start-2 col-end-3 row-start-2 row-end-2 mx-auto flex h-full min-h-10 w-0.5 justify-center rounded-full",
